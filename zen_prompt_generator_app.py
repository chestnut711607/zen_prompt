...
STYLE_CONFIG = {
    "wabi": {
        "name": "侘寂禅意",
        "label": "侘寂禅意｜灰白米色、石色、低饱和，安静留白",
        "description": "灰白、米色、石头本色与低饱和大地色，画面克制、空灵、安静，适合表达留白、孤石、枯山水和静默感。",
    ...
    "tea": {
        "name": "茶禅",
        "label": "茶禅｜木色、茶褐、柔和米色，温暖静心",
        "description": "木色、茶褐、柔和米色和淡绿色，带一点窗边暖光，适合茶席、香、读书、独处和慢生活画面。",
    ...
    "life": {
        "name": "生命禅意",
        "label": "生命禅意｜暖阳、白云、新叶绿、天空蓝，清新有生机",
        "description": "暖阳、白云、新叶绿、柔和天空蓝、暖白和浅大地色，整体明亮、有陪伴感和生命力，适合成长、希望、传承、更新与自然生态主题。",
        "colors": [
            "warm golden sunlight",
            "soft white clouds",
            "fresh new-leaf green",
            "soft sky blue",
            "warm white",
            "gentle spring green",
            "light earth tones",
    ...
    "family": {
        "name": "家庭禅意",
        "label": "家庭禅意｜暖阳、柔绿、暖白，安定陪伴感",
        "description": "暖阳色、柔和绿色、暖白和大地色，画面温柔、安定、有陪伴感，适合亲子、家庭、团圆与传承主题。",
    ...
    "academy": {
        "name": "静心生态",
        "label": "静心生态｜竹绿、晨光金、浅玉色，秩序与信任",
        "description": "清新的竹绿、晨光金、浅玉色和柔和白云色，画面更有秩序、连接感和长期信任感，适合学堂、小院、治理、合规和生态主题。",
    ...
    "culture": {
        "name": "文化禅意",
        "label": "文化禅意｜水墨灰、纸白、木色，清雅有书卷气",
        "description": "水墨灰、纸白、木色、低饱和绿色和淡金光，气质清雅、沉静、有书卷气，适合智慧、课程、讲座和文化主题。",
...
def style_label_to_key(style_label: str):
    for key, config in STYLE_CONFIG.items():
        if style_label in (config["label"], config["name"]):
            return key
    return None
...
def generate_prompt(keyword, poster=True, vertical=True, style_override="自动识别"):
    ...
    if style_override != "自动识别":
        style_key = style_label_to_key(style_override) or style_key
    ...
    return ",\n".join(prompt_parts), config["name"], subject, composition, config["description"]
...
style_options = ["自动识别"] + [STYLE_CONFIG[k]["label"] for k in STYLE_CONFIG]
...
if st.button("生成提示词", type="primary"):
    if not keyword.strip():
        ...
        prompt, style_name, subject, composition, style_description = generate_prompt(
            keyword.strip(),
            poster=poster_mode,
        ...
        st.write(f"**风格描述：** {style_description}")
